// Package mirrormark — L43 Mirror-Mark v1 signer for moneycheck.
//
// 2026-05-27 new-flagship-inception ship. Pure-stdlib (`crypto/hmac` +
// `crypto/sha256` + `encoding/base64`); zero external deps.
// Byte-identical algorithm to `foundation/pkg/mirrormark` and to the
// ~30+ Mirror-Mark v1 cohort flagships.
//
// Cohort role: moneycheck stamps Mirror-Mark receipts onto audit-ledger
// entries from inception (R175 production-wired). A downstream FCA /
// NCA inspector holding (lore.tar.gz, the emitted entry bytes with
// `mirror_mark` cleared, and the PSP's registered iik_ key) can
// independently re-derive the mark and confirm both:
//
//  1. the corpus SHA in the mark matches a clean re-hash of
//     lore.tar.gz, AND
//  2. the HMAC over (markVersion || corpusSHA || payload) matches
//     the value moneycheck emitted.
//
// R175 R-MIRROR-MARK-LOAD-BEARING-IN-PRODUCTION (godfather memory
// 2026-05-27): moneycheck satisfies all four R175 criteria from
// inception:
//
//  1. Production-traffic emit-path: `internal/audit_ledger/ledger.go`
//     Emit() calls `marker.Sign()` on every entry (non-test code).
//  2. Cold-verify path: OpenSSL one-liner reproduces KAT-1 hex per
//     `internal/lore/kat1.go` doc-comment.
//  3. Boot-time R143 LOUD-ONCE-WARN: five named advisories disclose
//     placeholder state of PSR/AML/SCA/disclosure surfaces.
//  4. Cohort canonical KAT-1 hex pin: pinned in `internal/lore/kat1.go`
//     constant + `internal/firewall/firewall_test.go` firewall test.
//
// Mark format (byte-identical to cohort canonical):
//
...
// downstream reader (regulator portal, FCA / NCA inspector) identify
// a v1 Mirror-Mark and reject unknown versions. Byte-identical to
// every cohort port.
...
// key. Empty-key fail-closed: moneycheck refuses to emit marks with an
// empty key under production discipline (the KAT-1 anchor uses an
// empty key by canonical convention but that's a regulator-cold-
// verify shape, not a production-marking shape).
...
// the same (corpusSHA, payload, key) triple across all ~30+ Mirror-
// Mark v1 cohort ports.
...
// StdlibMarker is the moneycheck-resident pure-stdlib Marker
...
func NewPlaceholderMarker() *StdlibMarker {
	...
	return &StdlibMarker{
		corpusSHA:        zeroSHA,
		key:              []byte("iik_dev_MONEYCHECK_NOT_FOR_PRODUCTION"),
...
func (m *StdlibMarker) Sign(payload []byte) string {
	if m.usingPlaceholder {
		m.warnedOnce.Do(func() {
			log.Printf("[LOUD-ONCE-WARNING] mirrormark: signing with placeholder key + zero corpus SHA; emitted marks will NOT pass cold-verify against a real lore corpus / production key — moneycheck/internal/mirrormark placeholder mode")
		})
	}
