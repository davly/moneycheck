...
// moneycheck run-ledger into the Stele verified-trust spine
// (infrastructure/stele) via POST /v1/verdicts.
//
// 2026-06-12 flagship consumer wire to the spine (R145.B sibling-branch
// ship; see internal/firewall/ re-pins). This is a deliberately LOCAL
// mirror of the spine wire-contract — the same pattern as nexus's
// internal stele_client and ofgemwatch's internal/stele — because
// moneycheck's go.mod is stdlib-only (TestFirewall_NoExternalDeps) and
// MUST NOT import the stele Go module.
//
// OFF BY DEFAULT: the CLI anchors only when MONEYCHECK_STELE_URL is
...
//     audit_ledger.(*Ledger).SelfCheck(). A failed self-check seals
//     NOTHING and the CLI fails loudly.
//   - oracle_strength is "Self-Check" and the evidence sentence says
//     so explicitly: the oracle is moneycheck's own ledger verifier,
...
//     audit_ledger.(*Ledger).SelfCheck (JSON of each stamped Entry,
//     '\n'-terminated, in append order).
...
// entirely. This is the ONE env read moneycheck performs; the R145
// firewall pins it to a single call site in cmd/moneycheck.
const EnvURL = "MONEYCHECK_STELE_URL"

// Substrate is the spine substrate identifier for moneycheck's
// run-ledger anchors.
const Substrate = "flagships/moneycheck/audit-ledger"

// OracleStrengthSelfCheck is the honest oracle-strength label for a
// run-ledger anchor: the verifier is moneycheck itself.
...
// SelfChecker is the seam the anchor consumes (kept as a local
// interface so this package stays decoupled from audit_ledger and the
// seam is trivially fakeable in tests). It matches the signature of
// audit_ledger.(*Ledger).SelfCheck.
...
func NewRunAnchor(command string, entries int, ledgerDigest [sha256.Size]byte, sealedAt time.Time) Verdict {
	...
	return Verdict{
		...
		Evidence: fmt.Sprintf(
			"run self-check: %d entries, ledger digest %s; oracle is moneycheck's own ledger verifier (self-check, NOT an independent gauntlet)",
